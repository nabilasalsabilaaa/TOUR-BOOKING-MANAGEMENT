...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div class="mb-4 md:mb-0">
                <div class="flex items-center">
                    ...
                    <div>
                        {{-- Title dinamis: Create / Edit --}}
                        <h1 class="text-3xl font-bold text-gray-900">
                            {{ isset($tour) ? 'Edit Tour' : 'Create New Tour' }}
                        </h1>
                        {{-- Sub-title penjelasan singkat --}}
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="card-custom p-8">
            ...
            {{-- ================================
                 FORM TOUR
                 - Dipakai untuk Create & Edit
               ================================ --}}
            <form 
                action="{{ isset($tour) ? route('admin.tours.update', $tour) : route('admin.tours.store') }}" 
                method="POST" 
                enctype="multipart/form-data" 
                class="prevent-multiple-submit"
            >
                @csrf
                {{-- Method PUT hanya kalau edit --}}
                @if(isset($tour))
                    @method('PUT')
                @endif
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {{-- ====================================
                         KOLOM KIRI (lg: span 2)
                         Data utama tour: nama, deskripsi,
                         lokasi, harga, durasi, kapasitas,
                         dan status aktif.
                       ==================================== --}}
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                placeholder="Describe the tour experience, highlights, and what customers can expect"
                            >{{ old('description', $tour->description ?? '') }}</textarea>
                            <div class="flex justify-between items-center mt-1">
                                <p class="text-xs text-gray-500">
                                    Provide a compelling description to attract customers
                                </p>
                                {{-- Counter jumlah karakter deskripsi --}}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1">
                                    Maximum number of guests per schedule
                                </p>
                                @error('capacity')
                                    <p class="mt-2 text-sm text-error flex items-center">
                                        <i class="fas fa-exclamation-circle mr-2"></i>
                    ...
                    {{-- ====================================
                         KOLOM KANAN
                         Upload thumbnail, preview harga,
                         dan tips form.
                       ==================================== --}}
                    <div class="space-y-6">
                        {{-- Upload Thumbnail --}}
                        ...
                        <div id="image-preview-container" class="{{ isset($tour) && $tour->thumbnail ? '' : 'hidden' }}">
                            ...
                            <div class="relative">
                                ...
                                >
                                {{-- Tombol X untuk hapus preview upload --}}
                                <button
                                    type="button"
                                    id="remove-image"
                                    class="absolute top-2 right-2 bg-error text-white p-1 rounded-full hover:bg-red-600 transition-colors"
                                >
                        ...
                        @if(isset($tour) && $tour->thumbnail)
                            <div id="current-image-container">
                                ...
                                <p class="text-xs text-gray-500 mt-2">
                                    This is the current image. Upload a new one to replace it.
                                </p>
                            </div>
                        @endif

                        {{-- Kartu preview harga & durasi (seperti yang dilihat user) --}}
...
{{-- ================================
     ...
     (didorong ke stack @stack('styles'))
   ================================ --}}
@push('styles')
<style>
    :root {
    ...
    /* Checkbox custom untuk status aktif */
    .checkbox-custom:checked {
        background-color: var(--tb-primary);
        border-color: var(--tb-primary);
    }
    
    /* Animasi hover pada preview gambar */
    .image-preview {
    ...
    /* Kartu preview harga di sidebar kanan */
    .price-preview {
        background: #010d4c;
...
{{-- ================================
     ...
     - Preview harga & durasi
   ================================ --}}
@push('scripts')
<script>
    // ================================
    // 1. Character count untuk deskripsi
    // ================================
    ...
    // Trigger sekali agar nilai awal langsung tampil
    description.dispatchEvent(new Event('input'));

    // ================================
    // 2. Handling upload thumbnail
    //    - Click area -> input file
    //    - Drag & drop
    //    - Preview gambar
    //    - Remove preview
    // ================================
    ...
    // Input file via dialog
    thumbnailInput.addEventListener('change', function() {
        if (this.files && this.files[0]) {
            updateImagePreview(this.files[0]);
            
            // Jika mode edit: sembunyikan gambar lama
    ...
    // Tombol X untuk menghapus preview upload
    removeImageBtn.addEventListener('click', function() {
        thumbnailInput.value = '';
        imagePreviewContainer.classList.add('hidden');
        
        // Kalau mode edit, tampilkan lagi gambar lama
        if (currentImageContainer) {
            currentImageContainer.classList.remove('hidden');
        }
    });

    // Helper: set src preview dari file upload
    ...
    // ================================
    // 3. Preview harga & durasi
    //    (supaya admin bisa lihat bagaimana
    //     tampil ke user)
    // ================================
    ...
    // Inisialisasi preview saat halaman dibuka
    updatePricePreview();
